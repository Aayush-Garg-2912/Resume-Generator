...
const ResumePreview = () => {
  ...
  return (
    <div className="resume-preview bg-white w-full max-w-[210mm] aspect-[1/1.414] p-8 sm:p-12 text-black text-base font-serif relative leading-snug">
      {/* HEADER */}
      <header className="text-center mb-4">
        ...
        <div className="flex flex-wrap justify-center items-center gap-1 text-[15px] text-gray-800">
      ...
      {personalInfo?.summary && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Professional Summary</h3>
          <p className="text-[15px] text-justify leading-relaxed whitespace-pre-line">
            {personalInfo.summary}
          </p>
        </section>
      )}
      ...
      {education && education.length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Education</h3>
      ...
      {hasSkills && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-1 border border-black inline-block px-1">Technical Skills</h3>
      ...
      {experience && experience.length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Experience</h3>
          <div className="space-y-3">
            {experience.map((exp, idx) => (
              <div key={idx}>
                ...
                {exp.description && (
                  <ul className="list-disc pl-5 text-[15px] space-y-0.5 text-justify">
      ...
      {projects && projects.length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Projects</h3>
          <div className="space-y-3">
            {projects.map((proj, idx) => (
              <div key={idx}>
                <div className="mb-0.5 flex items-center gap-1">
                  <p className="font-bold text-base inline">
                    {proj.projectName}
                  </p>
                  {proj.githubLink && (
                    <a href={proj.githubLink.startsWith('http') ? proj.githubLink : `https://${proj.githubLink}`} target="_blank" rel="noreferrer" className="text-blue-600 underline text-[15px] ml-1">
                      GitHub
                    </a>
                  )}
                  {proj.liveDemo && (
                    <a href={proj.liveDemo.startsWith('http') ? proj.liveDemo : `https://${proj.liveDemo}`} target="_blank" rel="noreferrer" className="text-blue-600 underline text-[15px] ml-1">
                      Live
                    </a>
                  )}
                </div>
                {proj.techStack?.length > 0 && (
                  <p className="text-[15px] italic mb-1 text-gray-700">
      ...
      {certifications && certifications.length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Certifications</h3>
          <ul className="list-disc pl-5 text-[15px] space-y-0.5">
      ...
      {achievements && achievements.length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Achievements</h3>
          <ul className="list-disc pl-5 text-[15px] space-y-0.5">
      ...
      {extraActivities?.positionsOfResponsibility?.filter(p => p.trim()).length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Positions of Responsibility</h3>
          <ul className="list-disc pl-5 text-[15px] space-y-0.5">
      ...
      {extraActivities?.extraCurricular?.filter(e => e.trim()).length > 0 && (
        <section className="mb-4">
          <h3 className="font-bold text-base uppercase tracking-wider mb-2 border border-black inline-block px-1">Extra-Curricular Activities</h3>
          <ul className="list-disc pl-5 text-[15px] space-y-0.5">
